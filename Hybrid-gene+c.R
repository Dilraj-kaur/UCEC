...
genes <- scan(file = '//Users/dilrajkaur/Desktop/UCEC/newdata/input_genes/UCEC_genes9.csv', what = 'character', sep = ',')
...
out_file='/Users/dilrajkaur/Desktop/UCEC/newdata/results/out'+gsize+'_'+cutoff+'genes.csv'
...
for(i in seq(from=1, to=length(f), by=1))#no of cancers
{
  ...
  # if (('BNIP3L' %in% g1)&('TNFRSF12A' %in% g1)&('PSEN1' %in% g1))
  {
    ...
    data1$PI=PI+1*((data1$clinical_stage=="Stage III") | (data1$clinical_stage== "Stage IIIA")|(data1$clinical_stage=="Stage IIIB")|(data1$clinical_stage=="Stage IIIC")|(data1$clinical_stage=="Stage IIIC1")|(data1$clinical_stage=="Stage IIIC2")|(data1$clinical_stage=="Stage IV")|(data1$clinical_stage=="Stage IV")|(data1$clinical_stage=="Stage IVA")|(data1$clinical_stage=="Stage IVB"))
