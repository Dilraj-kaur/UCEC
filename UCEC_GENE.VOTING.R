...
genes <- scan(file = '/Users/dilrajkaur/Desktop/UCEC/input_genset/UCEC_genes12.csv', what = 'character', sep = ',')
PI1=0
PI=0
v=0
for(i in seq(from=1, to=length(genes), by=1))
{
  if (beta[beta$Gene==genes[i],][4][,1]<0.05)
  {
    ...
    if (k>0)
    {
      b=1*(dplyr::select(data1, genes[i])[,1]>median(dplyr::select(data1, genes[i])[,1])) # voting
      b1=k*(select(data1, genes[i])[,1])} #for PI
    
    if (k<0)
    {
      b=1*(dplyr::select(data1, genes[i])[,1]<median(dplyr::select(data1, genes[i])[,1])) # for voting
    b1=k*(select(data1, genes[i])[,1])} #for PI
...
fit1 <- survfit(surv_object~PI>6)

ggsurvplot(fit1, data=data2,xlab = "Time (Months)")

fit1.coxph<-coxph(surv_object~PI>6)
...
gg<-ggsurvplot(fit1, data=data2,xlab = "Time (Months)",legend.labs = c("Low Risk", "High Risk"),axes.offset=FALSE,)

ggsave('/Users/dilrajkaur/Desktop/UCEC/10.gene.model.jpg', plot = print(gg),height = 5 ,width = 7, dpi = 1000)



#########################################gene.PI model #######################################
#example for stage 3,4 vs stage 1,2
####################################for selecting cutoff for PI ################################
library("survMisc")
fit1<-Surv(time = data1$OS.time/30, event = data1$vital_status)
fit2 <- coxph(fit1~PI1, data = data1)
cut <- cutp(fit2)$PI1
...
#survival analysis: fits cox ph model to find HR for mean cut
fit1 <- survfit(surv_object~PI1>=g$PI1)

ggsurvplot(fit1, data=data2,xlab = "Time (Months)")

fit1.coxph<-coxph(surv_object~PI1>=g$PI1)
...
#no of samples,HR,p-val,C,%95 CI L,%95CI U,logrank-p
print(c(nrow(data2),first[2],first[5],a$concordance[1],a$conf.int[3],a$conf.int[4],a$logtest[3],fit1$n[1],fit1$n[2],g$PI1))
