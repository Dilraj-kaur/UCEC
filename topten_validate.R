...
library(dplyr)
  ...
  out_file='/Users/macbook/Desktop/UCEC/'+cancer+'_valid_'+val+'.csv'
  ...
  data1$PI=PI
  #data1$PI=PI+1*(data1$age_at_diagnosis>65)
  #g=length(genes)/2
...
#boxplot(HR1,HR2,HR3,ylim=c(0,100),ylab="Hazard Ratio (HR)", xlab="Sampling size",names = c('50%','70%','90%'),col=c("grey",'cyan','orange'))
boxplot(C1,C2,C3,ylab="C-Index",xlab="Sampling size",names = c('50%','70%','90%'),col=c("grey",'cyan','orange'))
